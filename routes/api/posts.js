...
router.get('/:id', (req, res) => {
    ...
    .catch(err => res.status(404).json({ noPostFound: 'no post found with the id ' +  req.params.id }));
})
...
router.delete('/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    .then(profile => {
        ...
        .then(post => {
            ...
            // Delete
            Post.remove(post).then(() => res.json({ success: true }));
        })
...
router.post('/comment/:id', passport.authenticate('jwt', { session: false}), (req,res) => {
    ...
    .catch(err => res.status(404).json({ postNotFound: 'No post fopuind'}));
})
